...
<?php
	if(isset($libs)){
        foreach($libs as $lib){
            if (file_exists(APPPATH."views/istas/headscripts/lib_{$lib}_scripts.php")){
                $this->load->view("istas/headscripts/lib_{$lib}_scripts.php");
            }
        }
    }
    if (isset($page) && file_exists(APPPATH."views/istas/headscripts/{$page}_scripts.php")){
        $this->load->view('istas/headscripts/'.$page.'_scripts');
	}
?>
